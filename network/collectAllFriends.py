...
if is_twitterId_in_db(userStart.id, edgeCol):
    ...
    try:
        ...
        for friend_id in friends:
            ...
            insert_edge(userStart.screen_name, userStart.id, friend.screen_name, friend.id, edgeCol)
    ...
    if is_twitterId_in_db(userID, edgeCol):
        ...
        try:
            for page in tweepy.Cursor(api.friends_ids, user_id=userID).pages():
                ...
                for friend_id in friends:
                    friend = api.get_user(user_id = friend_id)
                    insert_edge(user.screen_name, user.id, friend.screen_name, friend.id, edgeCol)
        except tweepy.TweepError:
            print("error")
            continue